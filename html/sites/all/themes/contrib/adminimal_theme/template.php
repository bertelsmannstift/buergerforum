...
function adminimal_preprocess_html(&$vars) {
  ...
  // Added class parent menu.
  $menuParent = menu_get_active_trail();
  $is_settings_page = FALSE;

  if (isset($menuParent[count($menuParent) - 1]['link_path']) && $menuParent[count($menuParent) - 1]['link_path'] ==
  ...
  if (isset($menuParent[count($menuParent) - 2]['link_title'])) {
    $menuParent = drupal_html_class($menuParent[count($menuParent) - 2]['link_title']);
    if (in_array($menuParent,array('settings','system','content-authoring','role-names-feminine-'))) {
      $is_settings_page = TRUE;
    }

    if ($is_settings_page) {
      $vars['classes_array'][] = 'parent-menu-' . $menuParent;
      drupal_add_js('
    (function ($) {
    function setHeightMenu() {
     $("#bf_admin_menu").height($("#bf_admin_menu .menu .menu").height() + 45);
    }
